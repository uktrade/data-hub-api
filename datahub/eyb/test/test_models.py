...
@pytest.mark.parametrize('eyb_lead_data', 'eyb_lead_factory',
    ({
        # EYB Triage data
        'triage_id': 96,
        'triage_hashed_uuid': 'b85dabe2a4c46828424d61ec99f496d91952f9e53733898ff5f0fee89f08b635',
...
class TestEYBLead:
    ...
    def test_db_instance_matches_factory_instance(self, eyb_lead_data, eyb_lead_factory):
        # eyb_lead_factory=EYBLeadFactory(**eyb_lead_data)
        assert EYBLead.objects.all().exists()
        eyb_lead_db = EYBLead.objects.get(pk = eyb_lead_factory.pk)
        verify_eyb_lead_data(eyb_lead_db, eyb_lead_data)

    # def test_str(self, eyb_lead_data):
