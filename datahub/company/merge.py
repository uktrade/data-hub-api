...
def _get_objects_from_configuration(configuration: MergeConfiguration, source: Union[Contact, Company]):
    """Gets objects for each configured field."""
    for field in configuration.fields:
        yield field, configuration.model.objects.filter(**{field: source})


def is_model_a_valid_merge_source(model: Union[Contact, Company], allowed_relations, dataStructure=Contact):
    ...
    relations = get_related_fields(dataStructure)
    disallowed_fields = []
    ...
    # models.
    self_referential_fields = get_self_referential_relations(dataStructure)
