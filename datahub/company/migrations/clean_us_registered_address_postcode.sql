...
set registered_address_postcode = sub.fix
from
	(select company_fix.id,
	 	(array_to_string(regexp_matches(replace(company_fix.registered_address_postcode, ' ', ''),
...
where
	...
	and sub.id = cmpy.id
	and cmpy.registered_address_postcode ~ e'^\\d{3}' = false
