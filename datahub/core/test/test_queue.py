...
def test_can_queue_one_thing(async_queue: DataHubQueue):
    async_queue.enqueue('test-short-running', Spy.queue_handler)
    async_queue.work('test-short-running')
    assert Spy.called
    assert Spy.params[0] == ()


def test_can_queue_one_thing_with_arguments(async_queue: DataHubQueue):
    async_queue.enqueue('test-short-running', Spy.queue_handler, 'arg1', 'arg2', test=True)
    async_queue.work('test-short-running')
    assert Spy.called
...
def test_can_process_multiple_queues_in_correct_priority_order(async_queue: DataHubQueue):
    async_queue.enqueue('dead-letter', Spy.queue_handler, 1)
    async_queue.enqueue('test-short-running', Spy.queue_handler, True)
    async_queue.enqueue('test-long-running', Spy.queue_handler, False)
    async_queue.work('test-short-running', 'test-long-running', 'dead-letter')
    assert Spy.times == 3
...
def test_datahub_enque_is_configured_with_correct_default_number_of_retries_and_intervals(
    ...
    retry_arg = datahub_queue_mock.call_args_list[-1][-1]['retry']
