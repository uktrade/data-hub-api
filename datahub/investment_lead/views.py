...
class EYBLeadViewSet(SoftDeleteCoreViewSet):
    ...
    ordering_fields = ['triage_created', 'triage_modified', 'company__name']
    ...
    def get_queryset(self):
        ...
        queryset = EYBLead.objects.select_related(
                'company',
                'address_country',
                'address_country__overseas_region',
                'sector',
            ).prefetch_related(
                'advisers',
                'investment_projects',
            ).filter(archived=False).exclude(
