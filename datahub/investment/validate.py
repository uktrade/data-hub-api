...
def validate(instance=None, update_data=None, fields=None, next_stage=False):
    ...
    """
    combiner = DataCombiner(instance, update_data)
    ...
    for field, req_stage in VALIDATION_MAPPING.items():
        ...
        if _field_incomplete(info, combiner, field):
    ...
    for field, rule in CONDITIONAL_VALIDATION_MAPPING.items():
        ...
        if _check_rule(info, combiner, rule) and _field_incomplete(info, combiner, field):
            errors[field] = REQUIRED_MESSAGE
...
def _field_incomplete(field_info, combiner, field):
...
def _check_rule(field_info, combiner, rule):
    """Checks a conditional validation rule."""
    if rule.field in field_info.relations:
        if field_info.relations[rule.field].to_many:
            actual_value = combiner.get_value_to_many(rule.field)
        else:
            actual_value = combiner.get_value_id(rule.field)
    else:
        actual_value = combiner.get_value(rule.field)
