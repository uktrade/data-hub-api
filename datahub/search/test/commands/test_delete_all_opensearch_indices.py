...
from datahub.search.management.commands import delete_all_opensearch_indices
...
def test_deletes_matching_indices(mock_opensearch_client, interactive, monkeypatch):
    ...
    management.call_command(
        delete_all_opensearch_indices.Command(),
        interactive=interactive,
    )

    mock_opensearch_client.return_value.cat.indices.assert_called_once_with(
        index='test-datahub-*',
        format='json',
    )
    mock_opensearch_client.return_value.indices.delete.assert_called_once_with(
...
def test_skips_deleting_if_no_matching_indices(mock_opensearch_client):
    ...
    management.call_command(delete_all_opensearch_indices.Command())

    mock_opensearch_client.return_value.indices.delete.assert_not_called()
...
def test_skips_deleting_if_not_confirmed(mock_opensearch_client, monkeypatch):
    ...
    management.call_command(delete_all_opensearch_indices.Command(), interactive=True)
