...
class Company(DocType, MapDBModelToDict):
    ...
    account_manager = dsl_utils.contact_mapping('account_manager')
    alias = String()
    archived = Boolean()
    archived_by = dsl_utils.contact_mapping('archived_by')
    contacts = dsl_utils.contact_mapping('contacts')
    archived_on = Date()
    ...
    one_list_account_owner = dsl_utils.contact_mapping('one_list_account_owner')
