...
class TestPaymentGatewaySessionManager:
    ...
    def test_refresh_from_govuk_pay_with_unfiltered_qs(self):
        """
        Test that the method calls `session.refresh_from_govuk_payment` for
        each object in the unfiltered queryset.
        """
        order1, order2 = OrderWithAcceptedQuoteFactory.create_batch(2)

        PaymentGatewaySessionFactory.create_batch(
            2,
            order=order1,
            status=factory.Iterator([
                PaymentGatewaySessionStatus.created,
        ...
        with mock.patch.object(
            PaymentGatewaySession.objects.model, 'refresh_from_govuk_payment'
        ) as mocked_method:
            PaymentGatewaySession.objects.refresh_from_govuk_pay()

            assert mocked_method.call_count == 5

    def test_refresh_from_govuk_pay_with_filtered_qs(self):
        """
        Test that the method calls `session.refresh_from_govuk_payment` for
        each object in the filtered queryset.
        """
        order1, order2 = OrderWithAcceptedQuoteFactory.create_batch(2)
        ...
        with mock.patch.object(
            ...
            PaymentGatewaySession.objects.filter(order=order1).refresh_from_govuk_pay()
