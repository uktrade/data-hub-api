# Generated by Django 3.2.20 on 2023-09-22 10:16

from django.conf import settings
from django.db import migrations, models
import django.db.models.deletion
import uuid


class Migration(migrations.Migration):

    dependencies = [
        migrations.swappable_dependency(settings.AUTH_USER_MODEL),
        ('company', '0137_contact_email_flag'),
        ('investment', '0012_update_specific_programmes'),
        ('task', '0003_alter_investmentprojecttask_task'),
        ('reminder', '0008_create_reminder_for_new_export_interactions'),
    ]

    operations = [
        migrations.CreateModel(
            name='UpcomingInvestmentProjectTaskReminder',
            fields=[
                ('id', models.UUIDField(default=uuid.uuid4, primary_key=True, serialize=False)),
                ('created_on', models.DateTimeField(auto_now_add=True)),
                ('modified_on', models.DateTimeField(auto_now=True, null=True)),
                ('event', models.CharField(max_length=255)),
                ('status', models.CharField(blank=True, choices=[('live', 'Live'), ('dismissed', 'Dismissed')], default='live', max_length=255)),
                ('email_notification_id', models.UUIDField(blank=True, null=True)),
                ('email_delivery_status', models.CharField(blank=True, choices=[('sending', 'Sending'), ('delivered', 'Delivered'), ('permanent-failure', 'Permanent failure'), ('temporary-failure', 'Temporary failure'), ('technical-failure', 'Technical failure'), ('unknown', 'Unknown')], default='unknown', help_text='Email delivery status', max_length=255)),
                ('adviser', models.ForeignKey(on_delete=django.db.models.deletion.CASCADE, related_name='+', to=settings.AUTH_USER_MODEL)),
                ('company', models.ForeignKey(on_delete=django.db.models.deletion.CASCADE, related_name='upcoming_investment_project_task_reminder', to='company.company')),
                ('project', models.ForeignKey(on_delete=django.db.models.deletion.CASCADE, related_name='upcoming_investment_project_task_reminder', to='investment.investmentproject')),
                ('task', models.ForeignKey(on_delete=django.db.models.deletion.CASCADE, related_name='upcoming_investment_project_task_reminder', to='task.task')),
            ],
            options={
                'abstract': False,
            },
        ),
    ]
