...
| `OPENSEARCH_POOL_MAXSIZE`                                             | No                                          | The [OpenSearch Python client max connection pool](https://github.com/opensearch-project/opensearch-py/blob/main/guides/connection_classes.md#connection-pooling)
(default=10).                                                                                                   |
| `PAAS_IP_ALLOWLIST`                                                   | No                                          | IP addresses (comma-separated) that can access the Hawk-authenticated endpoints.                                                                                                 |
| `REDBOX_ACCESS_KEY_ID`                                                | No                                          | A non-secret access key ID, corresponding to `REDBOX_SECRET_ACCESS_KEY`. The holder of the secret key can access the datasets endpoint by Hawk authentication.                   |
| `REDBOX_SECRET_ACCESS_KEY`                                            | If `REDBOX_ACCESS_KEY_ID` is set            | A secret key, corresponding to `REDBOX_ACCESS_KEY_ID`. The holder of this key can access the datasets endpoint by Hawk authentication.                                           |
